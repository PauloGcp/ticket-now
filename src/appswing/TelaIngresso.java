...
public class TelaIngresso {
	...
	private void initialize() {
		...
		button_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					IngressoIndividual i = Fachada.criarIngressoIndividual(Integer.parseInt(id));

					label_3.setText("Codigo:" + i.getCodigo());
					label_2.setText("Jogo:" + i.getJogo().getId());
		...
		button_2.setBounds(428, 9, 86, 23);
		...
		button_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					IngressoGrupo i = Fachada.criarIngressoGrupo(array);
					label_3.setText("Codigo:" + i.getCodigo());
		...
		try{
			...
			for(Ingresso ing : lista) {
				if(ing instanceof IngressoIndividual) {
					int id = ((IngressoIndividual) ing).getJogo().getId();
					model.addRow(new Object[]{"Individual" ,ing.getCodigo(), ing.calcularValor(), id});
				}
				else {
					texto="";
					for(Jogo j : ((IngressoGrupo)ing).getJogos()) 	//obter os id  dos jogos
						texto += j.getId()+ "," ;

					model.addRow(new Object[]{"Grupo" ,ing.getCodigo(), ing.calcularValor(), texto});
				}
			}
